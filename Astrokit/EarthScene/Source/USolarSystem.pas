...
(*
  ...
  Z "axis" around which the planets turn.
*)
(*
  Spec,{Tag: 1 Sun,2 Planet,3 Ast,4 Com,5 Deb,6 Ring,7 Moon, 8 3ds
  19 CometSprite 20 Sun Flare, 21 Ring Proxy, 22 FFDebris Proxy}
  Species,{Hint Parent#   ,sub#}
  SubSpecies{Object#}
  for Proxyi:=1 to trunc(DebrisDataTmpArray[i].Density) do
  begin
  DCSolarSystem.children[LevelCount].children[0].Children[Level2Count].children[Proxyi].
...
type
  ...
  TSunData = record // Also Equal to material texture
    { NOT 8.3 NAME only ..ONLY .jpg
      Expected: .bmp  .tif  .tga .png ignored }
    SunName: String[255];
    Radius: Double;
  ...
  // These are used to compute the above.. depending on the current time
...
begin
  ...
  begin
    ...
    repeat
      ...
      SinCosine(eA0 * cPIdiv180, se, ce);
      eccentricAnomaly := eA0 - (eA0 - e * c180divPI * se - M) / (1 - e * ce);
      Inc(nn);
    until (nn > 10) or (Abs(eccentricAnomaly - eA0) < 1E-4);

    SinCosine(eccentricAnomaly * cPIdiv180, se, ce);
    xv := a * (ce - e);
    yv := a * (Sqrt(1.0 - e * e) * se);
    v := ArcTan2(yv, xv) * c180divPI;
    r := Sqrt(xv * xv + yv * yv);
    SinCosine(i * cPIdiv180, si, ci);
    SinCosine(N * cPIdiv180, sn, cn);
    SinCosine((v + w) * cPIdiv180, svw, cvw);
  end;
